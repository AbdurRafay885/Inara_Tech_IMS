  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 bg-slate-900/30 border border-slate-900 rounded-xl p-4">
        ...
        <select
          className="glass-input py-2 text-sm"
          value={historyFilters.completionStatus}
          onChange={(e) => setHistoryFilters(prev => ({ ...prev, completionStatus: e.target.value }))}
        >
          <option className="bg-slate-950" value="">All Statuses</option>
          <option className="bg-slate-950" value="Completed">Completed</option>
          <option className="bg-slate-950" value="Terminated">Terminated</option>
        </select>
      </div>
      ...
      {(() => {
        ...
        const filtered = q
          ? historyRecords.filter(r =>
            ...
            (r.internEmail || '').toLowerCase().includes(q) ||
            (r.department || '').toLowerCase().includes(q)
          )
          : historyRecords;

        if (filtered.length === 0) return (
          <p className="text-slate-500 text-center py-8">No historical internship records found.</p>
        );

        return (
          <div className="grid grid-cols-1 gap-6">
            {filtered.slice((historyPage - 1) * ITEMS_PER_PAGE, (historyPage - 1) * ITEMS_PER_PAGE + ITEMS_PER_PAGE).map((record) => (
      ...
      {renderPagination(historyPage, (() => { const q = headerSearch.trim().toLowerCase(); return q ? historyRecords.filter(r => (r.internName || '').toLowerCase().includes(q) || (r.internEmail || '').toLowerCase().includes(q) || (r.department || '').toLowerCase().includes(q)).length : historyRecords.length; })(), setHistoryPage)}
    </div>
  );
};
